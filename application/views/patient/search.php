...
//                print_r($posted_data);
    ...
    <?php  
         $km = $this->input->get('km', TRUE);  
    
    ?>
    <li class="content_right_menu_select_category content_right_menu_select_radius">
        <div class="select-style">
            <select name ="km" id ="milessort">
                <option value ="5" <?php if( $km == "5" ) { echo "selected" ; } ?>>5 km </option>
                <option value ="10" <?php if( $km == "10" ) { echo "selected" ; } ?> >10 km </option>
                <option value ="15" <?php if( $km == "15" ) { echo "selected" ; } ?>>15 km </option>
                <option value ="20" <?php if( $km == "20" ) { echo "selected" ; } ?> >20 km </option>
                <option value ="25" <?php if( $km == "25" ) { echo "selected" ; } ?>>25 km</option>
            </select>
        </div>
    </li>
    <li class="content_right_menu_find_button"> <button type ="submit"  class="btn btn-" id="search">find</button></li>
</ul>
</form>
...
<div class="patient_select_style_left1">
    <div class="select-style orders-x">
...
<div class="patient_content_right_box_border_bottom">
    <div class="patient_content_right_box_top">
        <div class="patient_book_appointment_left"> <img src="<?php echo base_url()."/".$search['picture']?>"  style ="border-radius: 51%; width: 68px;"/>
            <div class="patient_book_appointment_left_text">
                <h1><?php echo $search['name']?></h1>
                <span><?php echo $search['title']?></span> </div>
        </div>
        <div class="patient_book_appointment_right">
            <h1>Price: $<?php echo $search['from_price']?> - $<?php echo $search['to_price']?>  </h1>
            <a href="#" class="fancybox getHrsSlots" data-toggle="modal" data-target="#modalslots" data-pr-id = "<?php echo $search['ID'] ?>" data-dtr ="<?php echo $search['userid'] ?>" >BOOK APPOINTMENT</a>
        </div>
    </div>
    ...
    <div class="patient_content_right_box_bottom">
        <div class="patient_content_right_box_eye_checkup"> <span>PROCEDURES</span>
            <h2><?php echo $search['procedure_name']?></h2>
...
<script type="text/javascript">
    ...
    var cord = '<?php echo json_encode($map); ?>';
        ...
        if(mapcord.length > 0){
            console.log(mapcord);
            function initialize(){
                var latlng = new google.maps.LatLng(mapcord[0][0],mapcord[0][1]);
                var myOptions = {
                    center: latlng,
                    zoom: 6,
                ...
                for(var k = 0 ; k < mapcord.length; k++){
                    ...
                    google.maps.event.addListener(marker, 'mouseover', (function(marker, k) {
                        return function() {
                            ...
                            var content="<div class='googlemap_min'><img src="+basurl+spList[k][1]+" style ='border-radius: 51%; width: 68px;'/><div class='googlemap_min_text'><h1>"+spList[k][0]+"</h1><a href='#' class='fancybox getHrsSlots' data-toggle='modal' data-target='#modalslots' data-pr-id='"+pid+"' data-dtr='"+usrid+"'>book appointment</a></div><div class='googlemap_min_icon'><i class='fa fa-star'></i><span>4.5</span></div>"
...
//            marker = new google.maps.Marker({
//                        position: new google.maps.LatLng(45.5378004, -73.7106865),
//                        offset: '0',
//                        map: map,
//                        center: latlng2,
//                       icon: {
